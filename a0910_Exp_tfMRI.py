...
def trainNet(net, SC_lamb,SC_u,FC,D,beta=0,maxIters=2001, lr=1e-2,useCorrLoss=True,FC_u=None):
    ...
    for i in range(len(combinations)):
        ...
        while loss>0.65 and attempt<1:
            ...
            trained_net, loss = trainNet(net, trainSC_lamb, trainSC_u, trainFC,trainSC_D,beta=beta,
                                 maxIters=2001, lr=1e-2, useCorrLoss=True, FC_u=None)
            #########  Test the results  ################
...
# In[]
        print('training bestnet')
        train_size = sc_train.shape[0]
        # validation_train_index = list(range(160,800))
        # validation_test_index = list(range(160))
        validation_train_index = list(range(int(train_size/5),train_size))
        validation_test_index = list(range(int(train_size/5)))
        # print("validation_train_index:", validation_train_index, "\num_of_nodes validation_test_index:", validation_test_index)
        sc_validation_train,sc_validation_test = sc_train[validation_train_index],sc_train[validation_test_index]
        fc_validation_train,fc_validation_test = fc_train[validation_train_index],fc_train[validation_test_index]
        bestnet = train_best_spectralNet(sc_validation_train,sc_validation_test,
        ...
        #
# In[]
#         print('Used hyperparameters:', bestnet.K, bestnet.num_hidden_layer, bestnet.activation)
        ...
        # break
    outfile = folder + 'result0910_beta1_' + file
    ...
    print(outtxt)
    file1 = open(folder + 'result0910_beta1.txt', "a")
    file1.write(outtxt + '\n')
    file1.close()
